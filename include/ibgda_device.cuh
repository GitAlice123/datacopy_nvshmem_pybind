...
nvshmemi_ibgda_poll_recv(int dst_pe, int qp_id) {
    ...
    *cq->cons_idx = old_cons_idx + 1;
    ...
    while ((static_cast<uint16_t>(old_cons_idx - HtoBE16(ld_na_relaxed(&cqe64->wqe_counter)) - 1) < ncqes)){
        // printf("SW cons_idx=%#lx, HW wqe_counter=%#x, ncqes=%u, qp_num=%u\n",
        //        old_cons_idx, HtoBE16(ld_na_relaxed(&cqe64->wqe_counter)), ncqes, qp_num);
        // Check if we have waited too long
        if (clock64() - start_time > 200000000000ull) {
            printf("[%d] nvshmemi_ibgda_poll_recv timeout for dst_pe=%d, qp_id=%d\n",
                   nvshmemi_device_state_d.mype, dst_pe, qp_id);
            trap();
        }
    }
    auto imm = HtoBE32(ld_na_relaxed(&cqe64->imm_inval_pkey));//int
    return imm;
}
...
__device__ static __forceinline__ void
nvshmemi_ibgda_put_nbi_thread(uint64_t req_rptr, uint64_t req_lptr, size_t bytes, int dst_pe, int qp_id, int message_idx) {
    // Get lkey and rkey, store them into lanes
    uint32_t num_wqes = 0;
    __be32 my_lkey = 0;
    uint64_t my_laddr = 0;
    ...
    while (remaining_bytes > 0) {
        my_chunk_size = min(remaining_bytes, ibgda_get_lkey_and_rkey(my_laddr = req_lptr, &my_lkey, req_rptr, dst_pe, &my_raddr, &my_rkey));
        // Move one more message
        auto chunk_size = my_chunk_size;
        remaining_bytes -= chunk_size;
        req_lptr += chunk_size;
        req_rptr += chunk_size;
        ++ num_wqes;
    }
    EP_DEVICE_ASSERT(num_wqes == 1);

    // Process WQE
    auto qp = ibgda_get_rc(dst_pe, qp_id);
    uint64_t base_wqe_idx = 0;
    base_wqe_idx = ibgda_reserve_wqe_slots(qp, num_wqes);
    auto wqe_ptr = ibgda_get_wqe_ptr(qp, base_wqe_idx);
    ibgda_write_rdma_write_wqe(qp, my_laddr, my_lkey, my_raddr, my_rkey, my_chunk_size,
                                   base_wqe_idx, &wqe_ptr);
