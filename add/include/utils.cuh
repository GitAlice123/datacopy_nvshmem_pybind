...
timeout_check(int **task_fifo_ptrs, int head, int rank, int expected, int tag = 0, int dispatch_iter = -1, int notify_iter = -1, bool is_cached_notify = false) {
    ...
    while (not_finished<kNumRanks>(task_fifo_ptrs[rank] + head, expected)) {
        if (clock64() - start_time > NUM_TIMEOUT_CYCLES and threadIdx.x == 0) {
            printf("DeepEP timeout check failed: %d (rank = %d, head = %d, dispatch_iter = %d, notify_iter = %d, is_cached_notify = %d)\n", 
                tag, rank, head, dispatch_iter, notify_iter, static_cast<int>(is_cached_notify));
            trap();
        }
    }
}
...
template <int kNumRanks>
__forceinline__ __device__ void
barrier_device(int **task_fifo_ptrs, int head, int rank, int tag = 0, int dispatch_iter = -1, int notify_iter = -1, bool is_cached_notify = false) {
    ...
    timeout_check<kNumRanks>(task_fifo_ptrs, head, rank, 0, tag, dispatch_iter, notify_iter, is_cached_notify);
}
